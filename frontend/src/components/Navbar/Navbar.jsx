  ...
  return (
    <>
                ...
                <div className='flex items-center' key={i}>
                  {
                    ...
                    className='px-4 py-1 border border-blue-500 hover:bg-white hover:text-zinc-800 transition-all duration-300' key={i} >
                      {items.title}
                      </Link> ): (
                      <Link to={items.link} 
      ...
      <div className={`${MobileNav} bg-zinc-800 h-screen absolute top-0 left-0 w-full z-40 flex flex-col items-center justify-center`} >
        {
          links.map((items, i) => (
            <Link to={items.link} className='text-white text-4xl mb-8 font-semibold hover:text-blue-500 transition-all duration-300' key={i}
            onClick={()=>MobileNav=== "hidden" ? setMobileNav("block") : setMobileNav("hidden")} >
              {items.title}
            </Link>
          ))
        ...
        {
          isLoggedIn==false && <>
          (<Link to="/login" className='px-8 py-2 mb-8 text-3xl font-semibold border border-blue-500 text-white hover:bg-white hover:text-zinc-800 transition-all duration-300'
          onClick={()=>MobileNav=== "hidden" ? setMobileNav("block") : setMobileNav("hidden")}>
            LogIn
          </Link>
          <Link to="/signup" className='px-8 py-2 mb-8 text-3xl font-semibold bg-blue-500 rounded hover:bg-white hover:text-zinc-800 transition-all duration-300'
          onClick={()=>MobileNav=== "hidden" ? setMobileNav("block") : setMobileNav("hidden")}>
            SignUp
          </Link>)</>
        }
