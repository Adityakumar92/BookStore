...
function BookCard({data, favourite}) {
  ...
  const handleRemoveBook = async () => {
    const response = await axios.put(
      "http://localhost:1000/api/v1/remove-book-to-favurite",
      {},
      {headers}
    );
    ...
    <div className='bg-zinc-800 rounded p-4 flex flex-col'>
        <Link to={`/view-book-details/${data._id}`}>
            <div className=''>
                ...
                <p className='mt-4 text-zinc-200 text-xl font-semibold'> ₹ {data.price}</p>
            </div>
        </Link>
        {favourite && (
          <button
          className='bg-yellow-100 px-4 py-2 rounded border-yellow-500 text-yellow-500 mt-4 '
